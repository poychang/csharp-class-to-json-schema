...
```html
<script type="module" src="./src/csharp-json-schema-converter.js"></script>
```
...
如果要輸出 MAF response format，改載入 MAF 元件並放入 `<csharp-json-schema-maf-converter>`：

```html
<script type="module" src="./src/csharp-json-schema-maf-converter.js"></script>

<csharp-json-schema-maf-converter></csharp-json-schema-maf-converter>
```

MAF 元件會額外顯示 `name` 與 `description` 欄位，`name` 必須為 1 到 64 個英數字、底線或 dash，`description` 不可為空。輸出會是下列結構，其中 `strict` 固定為 `true`：
...
如果只是本機試用，也可以直接用瀏覽器開啟 [index.html](./index.html)，目前預設載入 MAF response format 轉換器。

## 功能

- 支援解析 `class`、`record class`、`record`、巢狀模型型別與 `enum`。
- 預設會將 C# 成員名稱轉為 camelCase；若有 `[JsonPropertyName]`，會優先使用 attribute 指定的名稱。
...
- public field 預設不會納入；透過 JavaScript API 傳入 `{ includeFields: true }` 時才會解析。
